...
class threadWorker implements Runnable
{
	...
	public void run()	
    {
		try
		{
            ...
            int responseID = parseQuery(tokenizer.nextToken());
            ...
            if (responseID == 1 || responseID == 5) //services GET or HEAD
            {
                ...
                else
                {
                    ...
                    outServer.close();
                    inServer.close();
                    System.out.println("Request processed: Bad Request");
                    return;
                }
                if (tokenizer.hasMoreTokens())
                {
                            ...
                            if (versionNum.charAt(5) == '1')
                            {
                                if (versionNum.charAt(6) == '.')
                                {
                                    if (versionNum.charAt(7) != '0')
                                    {
                                        ...
                                        outServer.close();
                                        inServer.close();
                                        System.out.println("Request processed: Not Supported");
                                        return;
                                    }
                                }
                                else
            ...
            else if (responseID == 2) //service POST
            {
                ...
                try 
                {
                    ...
                    byte[] b1 = contentLength.substring(16,contentLength.length()-1).getBytes();
                    System.out.println("Writing");
                    BufferedWriter writer = new BufferedWriter(new OutputStreamWriter(stdout));
                    writer.write(contentLength.substring(16,contentLength.length()-1));
                    writer.flush();
                    writer.close();
                              ...
                              byte[] payload)
    {
        ...
        try 
        {
            ...
            outServer.write(payload,0,payload.length);       
            outServer.flush();
        } 
        catch (IOException e)
        {
